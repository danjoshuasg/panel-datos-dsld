...
import { AtSign, ArrowRight, ArrowLeft, CheckCircle, Shield } from "lucide-react"

interface RecoverFormProps {
  onSwitchView: (view: "login" | "register" | "recover") => void
}

export function RecoverForm({ onSwitchView }: RecoverFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Here you would handle actual password recovery logic
    ...
    <div className="bg-gradient-to-b from-gray-50 to-gray-100 p-8 rounded-2xl border border-gray-200/50 shadow-xl backdrop-blur-sm backdrop-filter relative overflow-hidden fade-in">
      ...
      <div className="relative z-10">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-800 slide-up">Recuperar Contraseña</h1>
          <p className="mt-2 text-gray-600 slide-up">Panel de Datos de las DEMUNA - MIMP</p>
          <p className="text-sm text-gray-500 slide-up">Te enviaremos un enlace para restablecer tu contraseña</p>
        </div>
        ...
        {!isSuccess ? (
          <form ref={formRef} onSubmit={handleSubmit}>
            <div className="space-y-6">
              ...
              <Button
                type="submit"
                className={`w-full bg-[#9b0000] hover:bg-[#6b0000] text-white staggered-item staggered-item-2 ${isPulsing ? "pulse" : ""}`}
                disabled={isLoading}
              >
                {isLoading ? (
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full border-gray-300 bg-white text-gray-700 hover:bg-white hover:border-gray-400 hover:text-gray-900 transition-colors staggered-item staggered-item-3"
                onClick={() => {
                  console.log("Back to login clicked from recover")
                  onSwitchView("login")
                }}
              >
          ...
          <div className="space-y-6 scale-in">
            <div className="flex flex-col items-center justify-center space-y-4 text-center">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-medium text-gray-800">Revisa tu correo</h3>
